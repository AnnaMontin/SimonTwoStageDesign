...
fixedDesign <- function(alpha, beta, p0, p1, nmin, nmax){
  ...
  return(list(N = acc[2,"N"], R = acc[2,"R"]))
}
...
SimonAdj <- function(alpha, beta, p0, p1, nmin, nmax, n1, X, lev, out, R, N){
 # out <- SimonTwoSD(nmax = nmax, nmin = nmin, p0 = p0, p1 = p1, alpha = alpha, beta = beta)
  ...
  return(list(R_old = R_old, R_ = R, N_old = N_old, R_new = NR_new[1,1], N_new = NR_new[1,2]))
}
...
print_regions_2 <- function(N, R, p, lev1, lev2, string, res = FALSE){
  ...
  plot(obj_cp[,1], obj_cp[,2], xlim = c(0, N), ylim = c(0, R),
       xlab = "# Patients", ylab = "# Responses", main = string, pch = 19, cex = 0.5)
